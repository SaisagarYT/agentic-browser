...
| `actuation.uploadFiles` | Populate an `<input type="file">` element with local file paths. | `{ contextId, elementRef: SharedReference, filePaths: string[] }` | `{ success: boolean, filesAssigned: number }` | **Critical** (Host file access) | BiDi (`input.setFiles`) | **VERIFIED** (P50: 2.0 ms; supports visible, hidden, dynamic, and cross-origin iframes) |
| `actuation.download` | Intercept and direct an incoming browser download stream to a path. | `{ contextId, destinationFolder: string, timeoutMs: number }` | `{ filePath: string, sizeBytes: number, mimeType: string }` | **Critical** (Arbitrary binary on host) | BiDi (`browser.setDownloadBehavior`) + Watchdog | **PARTIALLY VERIFIED** (Folder sandboxing verified; BiDi completion events missing, requires external file watcher) |
